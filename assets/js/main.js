...
$("body").on("click", "[data-id][data-cliente]", function (e) {
    e.preventDefault();
    var botao = $(this).parent();
    var useBotao = botao.prevObject[0].classList[0];
    var select = botao.prevObject[0].dataset;
    var pix = select["id"];
    var cliente = select["cliente"];

    if (useBotao === "qrcopy") {
        console.log("saofnoiasjosajd");
        textInput.select();
        document.execCommand('copy');
    }

    if (useBotao === "anchorID") {
        let botao = `<button data-cliente="` + cliente + `" data-id="` + pix + `" class='geraQr btn btn-success' id="gerapix">Gerar Pix</button>`;
        $.ajax({
            url: '#',
            success: function (response) {
                ...
                $("#textqr").remove();
                $("#qrCodeArea").remove();
                $("#divModal").prepend(botao);
                openModalPix(pix,cliente);
            },
